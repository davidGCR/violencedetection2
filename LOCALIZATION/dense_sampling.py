...
def __main__ ():
    ...
    dataloaders_dict, test_names = anomalyInitializeDataset.dataloader_test_dense_dataset(path_dataset, train_videos_path, test_videos_path,
                                    batch_size, num_workers, numDiPerVideos, transforms_dataset, maxNumFramesOnVideo, videoSegmentLength,
                                    positionSegment, shuffle, patch_size)
    ...
    for i, data in enumerate(dataloaders_dict['test'], 0):
        ...
        dis_images = torch.squeeze(dis_images,dim=0)
        dis_images = dis_images / 2 + 0.5
        pytorch_show(make_grid(dis_images.cpu().data, nrow=7, padding=10))
        plt.show()
        
        patches_resized = []
        for idx in range(dis_images.size()[0]):
            patch = resize_transform(dis_images[idx].cpu())
            patches_resized.append(patch)
        
        patches_resized = torch.stack(patches_resized, dim=0)
        patches_resized = patches_resized.to(device)

        print('patches_resized: ', patches_resized.size())

        # scores = []
        with torch.set_grad_enabled(False):
            outputs_patches = classifier(patches_resized)
        ...
        for idx in range(dis_images.size()[0]):
            print('patch: ', idx, ', label_predict: ', str(indices_patches[idx].data))
            print('val: ', str(values_patches[idx].data), ', score: ', str(p[idx].data))
            patch = patches_resized[idx]
            pytorch_show(make_grid(patch.cpu().data, nrow=7, padding=10))
            plt.show()
        ############### PLOT ##################
