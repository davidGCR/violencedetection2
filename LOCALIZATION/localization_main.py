...
def plotOpencv(video_name, no_segment, prediction, real_frames, gt_bboxes, persons_in_segment, bbox_predictions, dynamic_image, mascara, preprocess, saliency_bboxes, sep, delay):
    red = (0, 0, 255)
    green = (0, 255, 0)
    blue = (255, 0, 0)
    yellow = (0, 255, 255)
    ...
    for idx_video, data in enumerate(anomalyDataset):
        ...
        if only_video_name is not None and indx_flac==0:
            if indx_flac==0:
                ...
                if idx is not None:
                    data = anomalyDataset[idx]
                    vid_name, label, numFrames = data
                    idx_video = idx
                    print('TEsting only one video {}, numFrames'.format(only_video_name,numFrames))
                else:
                    print('No valid video...')
            else:
                break
...
def train_test_videos(path_test_violence, path_violent_frames, min_num_frames):
    ...
    print('Test Split: ',len(test_names), len(test_labels), len(NumFrames_test), len(test_bbox_files))
    data = {
...
def __main__():
    ...
    classifier_file = '/Users/davidchoqueluqueroman/Desktop/PAPERS-CODIGOS/violencedetection2/ANOMALY_RESULTS/checkpoints/Model-resnet18, segmentLen-20, numDynIms-6, frameSkip-0, epochs-10, new_split-False, split_type-train-test'
