...
def main():
    ...
    # video_file_path = os.path.join(constants.PATH_HOCKEY_VIDEOS, "angela×fripSide - Boku wa Boku de Atte (1280x720 h264 AAC).mp4")
    # _, video_name = os.path.split(video_file_path)

    # #Call the public key-frame extraction method
    # imgs = vd.extract_frames_as_images(no_of_frames=no_of_frames_to_returned, file_path=video_file_path)
    
    img_module = ImageSelector(None)

    images_folder = os.path.join(constants.PATH_HOCKEY_FRAMES_VIOLENCE, '1')
    _, video_name = os.path.split(images_folder)
    imgs_paths = os.listdir(images_folder)
    frames = []
    for i, path in enumerate(imgs_paths):
        frame_path = os.path.join(images_folder, path)
    ...
    output_folder_video_image = os.path.join(constants.PATH_HOCKEY_VIDEOS,'selectedframes',video_name)
    if not os.path.exists(output_folder_video_image):
        os.mkdir(output_folder_video_image)
    
    keyframes = img_module.select_best_frames(input_key_frames=frames, number_of_frames=15)

    print('Nunmmber of keyframes=',len(keyframes))

    # Save all frames to disk
    for counter,img in enumerate(keyframes):
